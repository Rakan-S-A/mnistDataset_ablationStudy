...
class NeuralNetMLP:
    ...
    def __init__(self, num_features, num_hidden, num_classes, dropout_rate=0, random_seed=123):
        super().__init__()
        
        self.num_classes = num_classes
        self.dropout_rate = dropout_rate  # dropout rate for hidden layer
    ...
    def forward(self, x, training=True):
        ...
        if training:
            ...
            self.dropout_mask = (np.random.rand(*a_h.shape) > self.dropout_rate) / (1.0 - self.dropout_rate)
            a_h *= self.dropout_mask
    ...
    def backward(self, x, a_h, a_out, y):
        ...
        # Gradient of MSE w.r.t output activations
        ...
        ### Hidden layer
        #########################
        d_loss__a_h = np.dot(delta_out, self.weight_out)  # backprop through output weights
        d_a_h__d_z_h = a_h * (1. - a_h)  # sigmoid derivative

        # Apply dropout mask to hidden layer gradient
        d_hidden = d_loss__a_h * d_a_h__d_z_h
        if hasattr(self, 'dropout_mask'):
            d_hidden *= self.dropout_mask

        # Gradients for hidden weights and biases
        d_loss__d_w_h = np.dot(d_hidden.T, x)
        d_loss__d_b_h = np.sum(d_hidden, axis=0)
...
def compute_mse_and_acc(nnet, X, y, num_labels=10, minibatch_size=100):
    ...
    for i, (features, targets) in enumerate(minibatch_gen):
        _, probas = nnet.forward(features, training=False)  # no dropout
        predicted_labels = np.argmax(probas, axis=1)

        onehot_targets = int_to_onehot(targets, num_labels=num_labels)
        batch_loss = np.mean((onehot_targets - probas)**2)
        mse += batch_loss
...
for e in range(num_epochs):
    ...
    for X_mini, y_mini in minibatch_gen:
        # Forward pass
        # During training
        a_h, a_out = model.forward(X_mini, training=True)
        # Backward pass
        d_loss__dw_out, d_loss__db_out, d_loss__d_w_h, d_loss__d_b_h = model.backward(X_mini, a_h, a_out, y_mini)
