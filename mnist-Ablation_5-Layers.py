...
class NeuralNetMLP5:
    ...
    def __init__(self, num_features, num_hidden, num_classes, random_seed=123):
        ...
        self.w4 = rng.normal(0.0, 0.1, (num_hidden, num_hidden))
        self.b4 = np.zeros(num_hidden)

        self.w5 = rng.normal(0.0, 0.1, (num_hidden, num_hidden))
        self.b5 = np.zeros(num_hidden)
    ...
    def forward(self, x):
        ...
        z4 = np.dot(a3, self.w4.T) + self.b4
        a4 = sigmoid(z4)

        z5 = np.dot(a4, self.w5.T) + self.b5
        a5 = sigmoid(z5)

        z_out = np.dot(a5, self.w_out.T) + self.b_out
        a_out = sigmoid(z_out)

        return (a1, a2, a3, a4, a5), a_out
    ...
    def backward(self, x, activations, a_out, y):
        ...
        dw_out = np.dot(delta_out.T, a5)
        db_out = np.sum(delta_out, axis=0)

        delta5 = np.dot(delta_out, self.w_out) * a5 * (1. - a5)
        delta4 = np.dot(delta5, self.w5) * a4 * (1. - a4)
        delta3 = np.dot(delta4, self.w4) * a3 * (1. - a3)
        delta2 = np.dot(delta3, self.w3) * a2 * (1. - a2)
        delta1 = np.dot(delta2, self.w2) * a1 * (1. - a1)

        dw5 = np.dot(delta5.T, a4)
        db5 = np.sum(delta5, axis=0)

        dw4 = np.dot(delta4.T, a3)
        ...
        return dw_out, db_out, dw5, db5, dw4, db4, dw3, db3, dw2, db2, dw1, db1





model = NeuralNetMLP5(num_features=28*28, num_hidden=100, num_classes=10)
        ...
        for X_train_mini, y_train_mini in minibatch_gen:
            ...
            (dw_out, db_out,
             ...
             dw1, db1) = model.backward(
                X_train_mini, activations, a_out, y_train_mini
            )
